...
get_link_git() {
  ...
  print_banner
  printf "${WHITE} 💻 Insira o link do GITHUB do seu ChatMaker que deseja instalar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " link_git
}
...
inquiry_options() {
  ...
  print_banner
  printf "${WHITE} 💻 Bem vindo(a) ao Gerenciador ChatMaker, selecione abaixo a proxima ação!${GRAY_LIGHT}"
  printf "\n\n"
  printf "   [0] Instalar ChatMaker\n"
  printf "   [1] Atualizar ChatMakerP\n"
  printf "   [2] Deletar ChatMaker\n"
  printf "   [3] Bloquear ChatMakerP\n"
  printf "   [4] Desbloquear ChatMaker\n"
  printf "   [5] Alter. dominio ChatMaker\n"
  printf "\n"
  read -p "> " option
